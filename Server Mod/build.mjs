...
async function main() {
    ...
    try {
        ...
        //Create a descriptive name for the zip package.
        const zipName = createZipName(packageJson);
        logger.log("success", `Zip name created: ${zipName}`);
        ...
        const zipFilePath = path.join(path.dirname(projectDir), `${zipName}.zip`);
        ...
        const zipFileInProjectDir = path.join(projectDir, `${zipName}.zip`);
...
function createZipName(packageJson){
    // Create a name for the zip using the project name and the release number.
    const name = packageJson.name;
